...
function clean(game) { return { code: game.code, phase: phase(game.round), round: game.round, status: game.status, players: game.players.map(({ token, ...p }) => ({ ...p, squad: squad(p) })), turn: game.players[game.turn]?.name, log: game.log.slice(-10), winner: game.winner, map: game.map, target: 12 }; }
...
function win(game, reason) { const ranked = [...game.players].sort((a,b) => b.score - a.score || b.insight - a.insight || squad(b) - squad(a)); game.status='finished'; game.winner = ranked[0].score === ranked[1].score ? 'A tie — both crews escape at moonrise.' : `${ranked[0].name} wins`; game.log.push(`${reason} ${game.winner}`); }
...
function action(game, token, kind) {
  ...
  if (kind === 'scout') { need(p, 1, 'Scouting'); p.insight += 1; game.log.push(`${p.name} sent 1 Yellow Pikmin to map a safe route (+1 route).`); }
...
module.exports={create,join,action,phase,squad,server};
